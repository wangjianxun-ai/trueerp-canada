...
async function renderPendingShipments(results,current,config){
  ...
  shipments.records.forEach(record=>{const id=record.关联单号||record.发货单号;if(!id)return;const sale=salesByOrder.get(record.关联单号)||{},update=overrides[record.关联单号];const raw=update?.status||record.状态;unique.set(id,{orderId:record.关联单号,shipmentId:record.发货单号,time:record.下单时间,warehouse:record.仓库,carrier:update?.carrier||record.物流公司,tracking:update?.tracking||record.物流单号,sku:record.货品编号,name:record.货品名称,spec:record.规格||sale.规格,barcode:record.货品条码||sale.条码,unit:record.单位,expiry:sale.保质期,qty:record.数量,status:String(raw).includes('已完成')||String(raw).includes('已发货')?'已发货':'未发货'})});
