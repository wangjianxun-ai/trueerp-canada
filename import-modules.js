...
async function renderImportedModule(config){
  ...
  const body=document.createElement('tbody');data.records.forEach(record=>{const row=document.createElement('tr');data.fields.forEach(field=>{if(field==='操作'){const cell=document.createElement('td');if(String(record.订单状态).includes('待审核'))cell.innerHTML=`<button class="btn primary" data-audit-order="${record.订单编号}">审核</button>`;else cell.textContent='-';row.appendChild(cell)}else if(config.kind==='sales'&&field==='订单编号'){const cell=document.createElement('td');cell.className='mono';const link=document.createElement('button');link.className='order-number-link';link.type='button';link.dataset.orderDetail=record.订单编号;link.textContent=valueText(record.订单编号);cell.appendChild(link);row.appendChild(cell)}else row.appendChild(createCell('td',record[field],field))});body.appendChild(row)});table.appendChild(body);wrap.appendChild(table);card.appendChild(wrap);
  ...
  results.querySelectorAll('[data-order-detail]').forEach(button=>button.onclick=()=>openOrderDetailPage(data.records.find(record=>String(record.订单编号)===button.dataset.orderDetail)));
}

function openOrderDetailPage(record){
  if(!record)return;const backdrop=document.createElement('div');backdrop.className='modal-backdrop order-detail-backdrop';backdrop.innerHTML=`<div class="modal order-detail-page"><div class="modal-head"><div><h2>订单详情</h2><small class="muted">订单编号：${valueText(record.订单编号)}</small></div><button type="button" class="close" data-close-order-detail>×</button></div><div class="order-detail-status"><span class="tag ${statusClass(record.订单状态)}">${valueText(record.订单状态)}</span><span>${valueText(record.销售渠道)}</span><span>${valueText(record.订单类型)}</span></div><div class="order-detail-section"><h3>订单信息</h3><div class="order-detail-grid"><div><span>网店订单号</span><b class="mono">${valueText(record.网店订单号)}</b></div><div><span>付款时间</span><b>${valueText(record.付款时间)}</b></div><div><span>处理时间</span><b>${valueText(record.处理时间)}</b></div><div><span>发货仓库</span><b>${valueText(record.发货仓库)}</b></div><div><span>物流公司</span><b>${valueText(record.物流公司)}</b></div><div><span>物流单号</span><b class="mono">${valueText(record.物流单号)}</b></div></div></div><div class="order-detail-section"><h3>货品明细</h3><div class="order-product-card"><div class="order-product-name">${valueText(record.货品名称)}</div><div class="order-detail-grid"><div><span>货品编号</span><b class="mono">${valueText(record.货品编号)}</b></div><div><span>规格</span><b>${valueText(record.规格)}</b></div><div><span>数量</span><b>${valueText(record.数量)}</b></div><div><span>保质期</span><b>${valueText(record.保质期)}</b></div><div><span>单价</span><b>${valueText(record.单价)}</b></div><div><span>金额</span><b>${valueText(record.金额)}</b></div></div></div></div><div class="order-detail-section"><h3>收货信息</h3><div class="order-detail-grid"><div><span>收货人</span><b>${valueText(record.收货人)}</b></div><div><span>手机</span><b>${valueText(record.手机)}</b></div><div class="wide"><span>收货地址</span><b>${valueText(record.收货地址)}</b></div><div><span>客户账号</span><b>${valueText(record.客户账号)}</b></div><div><span>应收合计</span><b>${valueText(record.应收合计)}</b></div></div></div><div class="modal-foot"><button type="button" class="btn primary" data-close-order-detail>关闭</button></div></div>`;document.body.appendChild(backdrop);backdrop.querySelectorAll('[data-close-order-detail]').forEach(button=>button.onclick=()=>backdrop.remove());backdrop.onclick=event=>{if(event.target===backdrop)backdrop.remove()};
}
...
async function renderPendingShipments(results,current,config){
  ...
  const salesByOrder=new Map();sales.records.forEach(record=>{if(record.订单编号&&!salesByOrder.has(record.订单编号))salesByOrder.set(record.订单编号,record)});
  shipments.records.forEach(record=>{const id=record.关联单号||record.发货单号;if(!id)return;const sale=salesByOrder.get(record.关联单号)||{},update=overrides[record.关联单号];const raw=update?.status||record.状态;unique.set(id,{orderId:record.关联单号,shipmentId:record.发货单号,time:record.下单时间,shippedAt:update?.shippedAt||record.发货时间||record.出库时间||'',warehouse:record.仓库,carrier:update?.carrier||record.物流公司,tracking:update?.tracking||record.物流单号,sku:record.货品编号,name:record.货品名称,spec:record.规格||sale.规格,barcode:record.货品条码||sale.条码,unit:record.单位,expiry:sale.保质期,qty:record.数量,status:String(raw).includes('已完成')||String(raw).includes('已发货')?'已发货':'未发货'})});
  sales.records.forEach(record=>{const id=record.订单编号,update=overrides[id];if(!id||unique.has(id)||update?.status!=='待发货')return;unique.set(id,{orderId:id,shipmentId:'-',time:record.付款时间,warehouse:record.发货仓库,carrier:'',tracking:'',sku:record.货品编号,name:record.货品名称,spec:record.规格,barcode:record.条码||'',unit:'',expiry:record.保质期,qty:record.数量,status:'未发货'})});
  ...
  wrap.innerHTML=`<table><thead><tr><th>发货单号</th><th>订单编号</th><th>下单时间</th><th>仓库</th><th>物流公司</th><th>物流单号</th><th>货品编号</th><th>货品名称</th><th>数量</th><th>状态</th><th>操作</th></tr></thead><tbody>${rows.map(record=>`<tr><td class="mono">${valueText(record.shipmentId)}</td><td class="mono">${valueText(record.orderId)}</td><td>${valueText(record.time)}</td><td>${valueText(record.warehouse)}</td><td>${valueText(record.carrier)}</td><td class="mono">${valueText(record.tracking)}</td><td class="mono">${valueText(record.sku)}</td><td>${valueText(record.name)}</td><td>${valueText(record.qty)}</td><td><span class="tag ${record.status==='已发货'?'green':'orange'}">${record.status}</span></td><td>${record.status==='未发货'?`<button class="btn primary" data-complete-shipment="${record.orderId}">发货</button>`:`<button class="btn" data-view-shipment="${record.orderId}">详情</button>`}</td></tr>`).join('')||'<tr><td colspan="11"><div class="empty">暂无符合条件的发货单</div></td></tr>'}</tbody></table>`;card.appendChild(wrap);
  ...
  results.querySelectorAll('[data-complete-shipment]').forEach(button=>button.onclick=()=>{const record=rows.find(item=>item.orderId===button.dataset.completeShipment);openShipmentDialog(record,()=>reload(current.page,current.query))});
  results.querySelectorAll('[data-view-shipment]').forEach(button=>button.onclick=()=>{const record=rows.find(item=>item.orderId===button.dataset.viewShipment);openShipmentDetailDialog(record)});
}

function openShipmentDetailDialog(record){
  if(!record)return;const backdrop=document.createElement('div');backdrop.className='modal-backdrop';backdrop.innerHTML=`<div class="modal shipment-modal"><div class="modal-head"><h2>发货单详情</h2><button type="button" class="close" data-close-shipment-detail>×</button></div><div class="shipment-summary"><div><span>发货单号</span><b class="mono">${valueText(record.shipmentId)}</b></div><div><span>订单编号</span><b class="mono">${valueText(record.orderId)}</b></div><div><span>下单时间</span><b>${valueText(record.time)}</b></div><div><span>发货时间</span><b>${valueText(record.shippedAt)}</b></div><div><span>发货仓库</span><b>${valueText(record.warehouse)}</b></div><div><span>发货状态</span><b>已发货</b></div></div><div class="shipment-product"><div class="shipment-product-title">货品明细</div><div class="shipment-product-grid detailed"><div><span>货品编号</span><b class="mono">${valueText(record.sku)}</b></div><div class="product-name"><span>货品名称</span><b>${valueText(record.name)}</b></div><div><span>产品条码</span><b class="mono">${valueText(record.barcode)}</b></div><div><span>产品规格</span><b>${valueText(record.spec)}</b></div><div><span>保质期</span><b>${valueText(record.expiry)}</b></div><div><span>发货数量</span><b class="shipment-qty">${valueText(record.qty)} ${record.unit||''}</b></div></div></div><div class="shipment-product"><div class="shipment-product-title">物流信息</div><div class="shipment-product-grid"><div><span>物流公司</span><b>${valueText(record.carrier)}</b></div><div><span>物流单号</span><b class="mono">${valueText(record.tracking)}</b></div></div></div><div class="modal-foot"><button type="button" class="btn primary" data-close-shipment-detail>关闭</button></div></div>`;document.body.appendChild(backdrop);backdrop.querySelectorAll('[data-close-shipment-detail]').forEach(button=>button.onclick=()=>backdrop.remove());backdrop.onclick=event=>{if(event.target===backdrop)backdrop.remove()};
}

function openShipmentDialog(record,onComplete){
  if(!record)return;const orderId=record.orderId;
  const backdrop=document.createElement('div');backdrop.className='modal-backdrop';backdrop.innerHTML=`<form class="modal shipment-modal" id="onlineShipmentForm"><div class="modal-head"><h2>订单发货</h2><button type="button" class="close" data-cancel-shipment>×</button></div><div class="shipment-summary"><div><span>订单编号</span><b class="mono">${valueText(record.orderId)}</b></div><div><span>发货单号</span><b class="mono">${valueText(record.shipmentId)}</b></div><div><span>下单时间</span><b>${valueText(record.time)}</b></div><div><span>发货仓库</span><b>${valueText(record.warehouse)}</b></div></div><div class="shipment-product"><div class="shipment-product-title">货品明细</div><div class="shipment-product-grid detailed"><div><span>货品编号</span><b class="mono">${valueText(record.sku)}</b></div><div class="product-name"><span>货品名称</span><b>${valueText(record.name)}</b></div><div><span>产品条码</span><b class="mono">${valueText(record.barcode)}</b></div><div><span>产品规格</span><b>${valueText(record.spec)}</b></div><div><span>保质期</span><b>${valueText(record.expiry)}</b></div><div><span>发货数量</span><b class="shipment-qty">${valueText(record.qty)} ${record.unit||''}</b></div></div></div><div class="form-grid shipment-form"><div class="field"><label>物流公司</label><select name="carrier"><option selected>天猫国际海外仓物流</option><option>Canada Post</option><option>UPS</option><option>FedEx</option></select></div><div class="field"><label>物流单号</label><input name="tracking" placeholder="扫描或输入物流单号" required autofocus></div></div><div class="modal-foot"><button type="button" class="btn" data-cancel-shipment>取消</button><button class="btn success">发货完成</button></div></form>`;document.body.appendChild(backdrop);
